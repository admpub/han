...
import (
	...
	"github.com/admpub/han/handler/mvc/static/resource"
	"github.com/admpub/han/handler/pprof"
	mw "github.com/admpub/han/middleware"
	"github.com/admpub/han/middleware/render"
	"github.com/admpub/han/middleware/render/driver"
	"github.com/admpub/han/middleware/tplfunc"
)
...
func HandlerWrapper(h interface{}) han.Handler {
...
func NewWithContext(name string, newContext func(*han.Han) han.Context, middlewares ...interface{}) (s *Application) {
	s = &Application{
		Name:           name,
	...
	if mwNum == 1 && middlewares[0] == nil {
		...
		s.DefaultMiddlewares = []interface{}{
			mw.Log(),
			mw.Recover(),
			mw.FuncMap(s.FuncMap, func(ctx han.Context) bool {
	...
	s.SessionOptions = &han.SessionOptions{
		Engine: `cookie`,
		Name:   `GOSID`,
	...
	s.ContextInitial = func(ctx han.Context, wrp *Wrapper, controller interface{}, actionName string) (err error, exit bool) {
		return ctx.(*Context).Init(wrp, controller, actionName)
	} //[1]
	if newContext == nil {
		newContext = func(e *han.Han) han.Context {
			return NewContext(s, han.NewContext(nil, nil, e))
		}
	}
	s.ContextCreator = newContext //[1]
	s.Core = han.NewWithContext(s.ContextCreator)
...
type Application struct {
	...
	ContextCreator     func(*han.Han) han.Context                                   `json:"-" xml:"-"`
	ContextInitial     func(han.Context, *Wrapper, interface{}, string) (error, bool) `json:"-" xml:"-"`
...
func (s *Application) SetDomain(name string, domain string) *Application {
	...
	a.Domain = domain
	a.Group = nil
	e := han.NewWithContext(s.ContextCreator)
...
func (s *Application) SetSessionOptions(sessionOptions *han.SessionOptions) *Application {
	if sessionOptions.CookieOptions == nil {
		sessionOptions.CookieOptions = &han.CookieOptions{
			Path:     `/`,
			HttpOnly: true,
		}
	}
...
func (s *Application) Tree(args ...*han.Han) (r map[string]map[string]map[string]map[string]string) {
	core := s.Core
	if len(args) > 0 {
		core = args[0]
	}
