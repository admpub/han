...
type Module struct {
	*Application       `json:"-" xml:"-"`
	Group              *han.Group   `json:"-" xml:"-"`
	Handler            *han.Han    `json:"-" xml:"-"` //指定域名时有效
...
func (a *Module) Register(p string, v interface{}, methods ...string) *Module {
	...
	a.Router().Match(methods, p, han.HandlerFunc(func(ctx han.Context) error {
		if c, y := ctx.(Initer); y {
...
func (a *Module) Router() han.ICore {
	if a.Group != nil {
		return a.Group
	}
	return a.Handler
...
func (a *Module) ExecAction(action string, t reflect.Type, v reflect.Value, c han.Context) (err error) {
	a.lock.Lock()
	...
	if methodName, ok := a.cachedHandlerNames[k]; ok {
		...
		if !valid {
			err = han.NewHTTPError(404, `invalid action: `+t.Name()+`#`+action)
			a.lock.Unlock()
			return
		}
