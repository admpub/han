...
var (
	...
	methodSuffixRegex  = regexp.MustCompile(`(?:_(?:` + strings.Join(han.Methods(), `|`) + `))+$`)
	routeTagRegex      = regexp.MustCompile(`^[A-Z.]+(\|[A-Z]+)*$`)
	DefaultMapperCheck = func(t reflect.Type) bool {
		return t == mapperType
	}
	DefaultContextInitial = func(ctx han.Context, wrp *Wrapper, controller interface{}, actionName string) (err error, exit bool) {
		return
	}
)
...
type BeforeHandler interface {
	Before(han.Context) error
}

//AfterHandler 静态实例中的后置行为
type AfterHandler interface {
	After(han.Context) error
}

//Initer 动态实例中的初始化行为
type Initer interface {
	Init(han.Context) error
}
...
func NewHandler(h func(han.Context) error, name string) *Handler {
	return &Handler{
		name:   name,
		handle: h,
	}
...
type Handler struct {
	name   string
	handle func(han.Context) error
}

func (h *Handler) Handle(c han.Context) error {
	return h.handle(c)
}
...
type Wrapper struct {
	// 静态实例中的行为
	beforeHandler han.HandlerFunc
	afterHandler  han.HandlerFunc
	// 动态实例中的行为状态
	hasBefore bool
	hasMain   bool
	hasAfter  bool
	...
	RouteRegister  han.RouteRegister `json:"-" xml:"-"`
	Module         *Module            `json:"-" xml:"-"`
	ControllerName string
}

func (a *Wrapper) wrapHandler(v interface{}, ctl string, act string) func(han.Context) error {
...
func (a *Wrapper) RouteTags() {
	if _, y := a.Controller.(Initer); !y {
		a.Module.Core.Logger().Infof(`%T is no method Init(han.Context)error, skiped.`, a.Controller)
		return
	}
	...
	for i := 0; i < e.NumField(); i++ {
		...
		*/
		...
		h := NewHandler(func(ctx han.Context) error {
...
func (a *Wrapper) execute(c han.Context, k string, e reflect.Type, u *URLs, action string) error {
...
func (a *Wrapper) Exec(ctx han.Context, t reflect.Type, action string) error {
	v := reflect.New(t)
	ac := v.Interface()
	if a.Module.ContextInitial != nil {
...
func (a *Wrapper) RouteMethods() {
	...
	for i := t.NumMethod() - 1; i >= 0; i-- {
		m := t.Method(i)
		name := m.Name
		h := func(k string, u *URLs) func(ctx han.Context) error {
			return func(ctx han.Context) error {
				return a.execute(ctx, k, e, u, name)
			}
		}
