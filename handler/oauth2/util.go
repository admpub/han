...
// we could use the han's sessions default, but this session should be not confict with the cookie session name defined by the sessions manager
...
*/
func BeginAuthHandler(ctx han.Context) error {
	url, err := GetAuthURL(ctx)
	if err != nil {
		return han.NewHTTPError(400, err.Error())
	}
...
// callback.
var SetState = func(ctx han.Context) string {
...
var GetState = func(ctx han.Context) string {
...
*/
func GetAuthURL(ctx han.Context) (string, error) {
	if ctx.Session() == nil {
...
as either "provider" or url query parameter ":provider".
*/
var CompleteUserAuth = func(ctx han.Context) (goth.User, error) {
...
func getProviderName(ctx han.Context) (string, error) {
