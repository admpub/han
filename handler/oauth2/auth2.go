...
type OAuth struct {
	...
	failHandler     han.HTTPErrorHandler
}
...
func (p *OAuth) Fail(handler han.HTTPErrorHandler) {
...
func (p *OAuth) Wrapper(e *han.Han) {
	...
	if len(oauthProviders) > 0 {
		...
		e.Get(p.Config.Path+"/login/:provider", func(ctx han.Context) error {
		...
		authMiddleware := func(h han.Handler) han.Handler {
			return han.HandlerFunc(func(ctx han.Context) error {
				user, err := CompleteUserAuth(ctx)
				if err != nil {
					return han.NewHTTPError(http.StatusUnauthorized, err.Error())
				}
		...
		if lastIndex == 0 {
			e.Get(p.Config.Path+"/callback/:provider", func(ctx han.Context) error {
