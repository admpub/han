...
func Captcha(opts ...*Options) func(han.Context) error {
	var o *Options
	if len(opts) > 0 {
		o = opts[0]
	}
	...
	return func(ctx han.Context) (err error) {
		var id, ext string
		param := ctx.P(0)
		...
		case ".png":
			...
			if download {
				header.Set(han.HeaderContentType, "application/octet-stream")
			} else {
				header.Set(han.HeaderContentType, "image/png")
			}
		...
		case ".wav":
			...
			if download {
				...
				header.Set(han.HeaderContentType, "audio/x-wav")
			}
