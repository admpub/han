...
import (
	...
	"github.com/admpub/han/middleware/bytes"
)
...
// G, T or P.
func BodyLimit(limit string) han.MiddlewareFunc {
	return BodyLimitWithConfig(BodyLimitConfig{Limit: limit})
}

// BodyLimitWithConfig returns a body limit middleware from config.
// See: `BodyLimit()`.
func BodyLimitWithConfig(config BodyLimitConfig) han.MiddlewareFunc {
	if config.Skipper == nil {
		config.Skipper = han.DefaultSkipper
	}
	...
	return func(next han.Handler) han.Handler {
		return han.HandlerFunc(func(c han.Context) error {
			...
			if req.Size() > config.limit {
				return han.ErrStatusRequestEntityTooLarge
			}
...
func (r *limitedReader) Read(b []byte) (n int, err error) {
	...
	r.read += int64(n)
	if r.read > r.limit {
		return n, han.ErrStatusRequestEntityTooLarge
	}
	return
}

func (r *limitedReader) Reset(reader io.Reader, context han.Context) {
	r.reader = reader
	r.context = context
}
