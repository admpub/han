...
type (
	...
	jwtExtractor func(han.Context) (string, error)
)
...
var (
	...
	DefaultJWTConfig = JWTConfig{
		...
		ContextKey:    "user",
		TokenLookup:   "header:" + han.HeaderAuthorization,
...
func JWT(key []byte) han.MiddlewareFuncd {
	c := DefaultJWTConfig
	c.SigningKey = key
...
// See: `JWT()`.
func JWTWithConfig(config JWTConfig) han.MiddlewareFuncd {
	// Defaults
	...
	return func(next han.Handler) han.HandlerFunc {
		return func(c han.Context) error {
			...
			if err != nil {
				return han.NewHTTPError(http.StatusBadRequest, err.Error())
			}
			token := new(jwt.Token)
			// Issue #647, #656
