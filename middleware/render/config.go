...
import (
	...
	"github.com/admpub/han/middleware"
...
func (t *Config) ApplyTo(e *han.Han, manager ...driver.Manager) *Config {
	if t.renderer != nil {
		t.renderer.Close()
	}
	e.SetHTTPErrorHandler(HTTPErrorHandler(t.ErrorPages))
	e.Use(middleware.FuncMap(tplfunc.New(), func(c han.Context) bool {
