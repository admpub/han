...
func (r *Request) RealIP() string {
	...
	if ip := r.header.Get(han.HeaderXForwardedFor); len(ip) > 0 {
		r.realIP = ip
	} else if ip := r.header.Get(han.HeaderXRealIP); len(ip) > 0 {
		r.realIP = ip
	} else {
...
func (r *Request) MultipartForm() *multipart.Form {
	if !strings.HasPrefix(string(r.context.Request.Header.ContentType()), han.MIMEMultipartForm) {
		return nil
	}
...
func (r *Request) BasicAuth() (username, password string, ok bool) {
	auth := r.Header().Get(han.HeaderAuthorization)
	if auth == "" {
		return
	}
